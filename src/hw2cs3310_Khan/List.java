...
class List {
     ...
     */
    static List push(Node lPtr, char newData) {
        if(lPtr == null) {
    ...
    /**
     * Counts the number of times a given character occurs in a list
     * @param searchFor the character occurrences which has to be counted
     * @return result of the counting
     */
    int count(char searchFor) {
        if (head == null)
            return 0;
        tempNodeHolder = head;
        int count = 0;
        while(tempNodeHolder.next != null) {
            if(searchFor == tempNodeHolder.nucleotide)
                count++;
            tempNodeHolder = tempNodeHolder.next;
        }
        return count;
    ...
    char getNth(int index) {
        ...
        return tempNodeHolder.nucleotide;
    }
     ...
     */
    void insertNth(int index, char data) {
        if (length() <= index || index < 0) {
